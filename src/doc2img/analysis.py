...
# what is the average Max CLIP score from the 
...
def quant_analysis(df, prompt_list):
    ...
    for index, row in df.iterrows():
        ...
        for prompt_score, prompt in prompt_dict.items():
            # Currently only doing this analysis on MAX CLIP Score 
            # Which is the first score in the tuple.
            scores = json.loads(row[prompt_score])
            if scores[0] > curr_best:
                curr_best = scores[0]
                best_prompt = prompt
    ...
    # Getting Average CLIP Score
    df['best_clip_max'] = best_scores
    print("Average Max CLIP Score: {}".format(np.average(best_scores)))
